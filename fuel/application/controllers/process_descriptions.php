...
class process_descriptions extends Fuel_base_controller {

    function __construct() {

        parent::__construct();
        $this->load->model('process_descriptions_model');
    ...
    function index() {
        ...
        $data['process_descriptions'] = $this->process_descriptions_model->list_items();

        $vars['body'] = $this->load->view('MGTSW/process_descriptions/index', $data, true);
...
//        $vars['body'] = $this->form_process($id);
    ...
    function form_process($id = null) {
        ...
        if (!empty($_POST)) {
            ...
            if ($this->_process($_POST)) {
                ...
                redirect(redirect('process_descriptions'));
            }
        }
        ...
        return $this->load->view('MGTSW/process_descriptions/form', $vars, TRUE);
    }
    ...
    function _process($data) {
        ...
        if ($this->validator->validate()) {
            ...
            if (empty($data['id']))
                $this->process_descriptions_model->insert($data);
            else {
                $whare = array('id' => $data['id']);
                unset($data['id']);
                $this->process_descriptions_model->update($data, $whare);
            }
            return TRUE;
        }
        return FALSE;
    ...
    function view($id = null) {
        ...
        $where = array();
        $where['id'] = $id;
        $customer_order = $this->process_descriptions_model->find_one_array($where);
        if (empty($customer_order['id'])) {
            show_404();
        }
        $data['customer_order'] = $customer_order;
        $vars['assets_path'] = $this->config->item('assets_path');
        $vars['body'] = $this->load->view('MGTSW/process_descriptions/view', $data, true);
    ...
    function remove($id = null) {
        ...
        $where = array();
        $where['id'] = $id;
        $customer_order = $this->process_descriptions_model->find_one($where);
        if (empty($customer_order->id)) {
            show_404();
        }
        if (!$this->fuel->auth->has_permission('process_descriptions_delete')) {
            show_error(lang('error_no_permissions'));
        }


        $this->process_descriptions_model->delete($where);
        redirect('process_descriptions');
    }
