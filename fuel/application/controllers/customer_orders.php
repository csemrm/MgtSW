...
class customer_orders extends Fuel_base_controller {
    ...
    function form_process($id = null) {
        ...
        if (!empty($id)) {
            $saved = $this->customer_orders_model->find_one_array(array('id' => $id));
            if (empty($saved))
                show_404();
        }
        ...
        $fields['po_proforma_file'] = array('required' => FALSE, 'type' => 'file', 'label' => 'PO and Pro-form Attached Files ', 'row_class' => 'create_a_customer');
        $fields['further_customer_file'] = array('required' => FALSE, 'type' => 'file', 'label' => 'Further Customer\'s Attached Files ', 'row_class' => 'create_a_customer');
