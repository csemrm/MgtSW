...
<div class="right_body_conten">
    ...
    <?php foreach ($sample_shipping_outs as $key => $sample_shipping_out):// print_obj($sample_shipping_out) ?>
        ...
        <div class="enabled">
            <?php if ($CI->fuel_auth->has_permission('sample_shipping_outs/edit')) { ?>
                <a class="update"href="<?php echo site_url('sample_shipping_outs/edit/' . $sample_shipping_out['id']) ?>">Update</a> | 
            <?php } if ($CI->fuel_auth->has_permission('sample_shipping_outs/remove')) { ?>
                <a class="remove" href="<?php echo site_url('sample_shipping_outs/remove/' . $sample_shipping_out['id']) ?>">Remove</a>
            <?php } ?>
        </div>
    <?php endforeach; ?>
